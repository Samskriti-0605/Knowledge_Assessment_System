...
switch($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            ...
            // Get assessments for a specific class and section
            $section = isset($_GET['section']) ? $_GET['section'] : null;
            if ($section) {
                $query = "SELECT a.*, u.name as created_by_name FROM assessments a JOIN users u ON a.created_by = u.id WHERE a.class_name = :class_name AND a.section = :section ORDER BY a.created_at DESC";
                $stmt = $db->prepare($query);
                $stmt->bindParam(":class_name", $_GET['class_name']);
                $stmt->bindParam(":section", $section);
            } else {
                $query = "SELECT a.*, u.name as created_by_name FROM assessments a JOIN users u ON a.created_by = u.id WHERE a.class_name = :class_name ORDER BY a.created_at DESC";
                $stmt = $db->prepare($query);
                $stmt->bindParam(":class_name", $_GET['class_name']);
            }
            $stmt->execute();
            ...
            // Get all assessments (for students)
            $query = "SELECT a.*, u.name as created_by_name FROM assessments a JOIN users u ON a.created_by = u.id ORDER BY a.created_at DESC";
            $stmt = $db->prepare($query);
            $stmt->execute();
            $assessments = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode($assessments);
    ...
    case 'POST':
        ...
        if (!empty($data->title) && !empty($data->created_by)) {
            $query = "INSERT INTO assessments SET title=:title, description=:description, created_by=:created_by, duration_minutes=:duration_minutes, class_name=:class_name, section=:section";
            $stmt = $db->prepare($query);
